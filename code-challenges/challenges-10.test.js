...
function greeting(require, response) {
  response.status(200).send('welcome');
}
function aboutme(require, response) {
  response.status(200).send('my name is sondos');
}
function favoritefoods(require, response) {
  response.status(200).send(['food', 'apple', 'pizza']);
}
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let arrOfarr = input.reduce((acc, val, ind) => {
    let arr = val.reduce((accu, value, index) => {
      if ( typeof(value)!=='string'&&value % 5 === 0) {
        accu.push(Math.pow(2, value));
      }
      return accu;
    }, []);
    acc.push(arr);
