...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const num=arr.reduce((accumilator,value,index)=>{
...
const returnNames = (arr) => {
  // Solution code here...
  let names=arr.reduce((accumilator,value,index)=>{
    accumilator.push(value.name);
    return accumilator;
  },[]);
  return names;
};
...
const reversedString = (str) => {
  ...
  for(let i=0;i< str.length;i++){
    newStr.unshift(str[i]);
     ...
     let reversed =newStr.reduce((accumilator,value,index)=>{
       accumilator=accumilator+ value;
        return accumilator;
